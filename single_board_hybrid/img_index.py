...
class gazebo_showup :
    def __init__(self, gazebo_path):
        self.img = cv2.imread('mother_board.png')
        ...
        t = threading.Thread(target=gazebo_showup.pose_listener())
        t.start()
        # path = [ (random.randint(0, 1023), random.randint(0, 553)) for i in range(30)]
        path = gazebo_path  # get a list of x y tuples
        print(path)
...
# rospy.Publisher('/KDD_RoboArm/nozzle_controller/command', Float64, queue_size = 1)]))
    ...
    def pixel2coordinate(p_index, val):
        ...
        y = (img.shape[:2][0] - p_index[1] + y_offset) * tf_y
        z = (val+z_bias)*tf_z
    ...
    def go_for_a_walk(seq, publishers):
        for i in seq:
            val = img[int(i[1])][img.shape[:2][0]-int(i[0])][0] #img[int_i[0]][int_i[1]][0]
    ...
    def pose_listener():
        rospy.Subscriber('/KDD_RoboArm/joint_states', JointState, gazebo_showup.pose_cb())
        rate=rospy.Rate(10)
